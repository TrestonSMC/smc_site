...
export default function AdminUploadPage() {
  ...
  const getSignedUploadToken = async (path: string) => {
    const res = await fetch("/api/admin/showroom-signed-upload", {
      method: "POST",
      headers: {
  ...
  const upload = async () => {
    ...
    try {
      const safeTitle = title.trim() || videoFile.name.replace(/\.[^/.]+$/, "");
      const id = slugify(safeTitle);
      ...
      await uploadWithSignedUrl(thumbnailBlob, thumbPath, "image/jpeg", (pct) =>
        setProgress(pct)
      );
